...
Now that we have a dataset comprising of images that contain an artificial region, we need to train our own model that detects such regions (one per image). Below is an example that illustrates all the steps made for a single image. This is an image from the test set, so here we can see a quite successful prediction from our final model.
